...
namespace ClassLibrary.Sever_connection.Mitsubishi_PLC
{
    ...
    abstract public class McProtocolApp : IPlc
    {
            ...
            switch (CommandFrame)
            {
                ...
                case McFrame.MC1E:
                    data = new List<byte>(6)
                   {
                    ...
                    break;
                default:
                    throw new Exception("The message frame isn't supported");
            }
            //TEST take care of the writing
            byte[] rtResponse = await TryExecution(sdCommand, length);
            int rtCode = Command.SetResponse(rtResponse);
        ...
        public async Task<byte[]> ReadDeviceBlock(string iDeviceName, int iSize)
        {
            PlcDeviceType type;
            int addr;
            GetDeviceCode(iDeviceName, out type, out addr);
            return await ReadDeviceBlock(type, addr, iSize);
        }
        ...
        public async Task<byte[]> ReadDeviceBlock(PlcDeviceType iType, int iAddress, int iSize)
        {
            ...
            //for (int i = 0; i < iValueNumber; i++)
            //{
            //    oData[i] = (rtCode == 0) ? Support.AutoBitConverter(rtData, i, iSize, iValueNumber) : 0;
            //}

            //if (rtCode == 0)
            //{                
            //    string s = ByteHelp.ByteToString(rtData, Encoding.ASCII).Replace("\0", "");
            //}            
            if (rtCode == 0)
            {
                return rtData;
        ...
        public async Task<string> ReadAndGetResult(string deviceName, int size, int valueType, int valueNumber = 1)
        {
            byte[] rtData = await ReadDeviceBlock(deviceName, size);
            ...
            {
                case 1:
                    var oData = new int[valueNumber];
                    for (int i = 0; i < valueNumber; i++)
                    {
                        oData[i] = Support.AutoBitConverter(rtData, i, size, valueNumber);
                    }
                    return string.Join(" ", oData);

                case 2:
                    string s;
                    s = Support.ByteArrToString(rtData, Encoding.ASCII).Replace("\0", "");
                    return s;
        ...
        /// <param name="iSize">读取长度：>0</param>
        /// <returns>数据</returns>
        //public async Task<int[]> ReadDeviceBlock(string iDeviceName, int iSize = 1, int iValueNumber = 1)
        //{
        //    int[] oData = new int[iValueNumber];
        //    await ReadDeviceBlock(iDeviceName, iSize, oData, iValueNumber);
        //    return oData;            
        //}
        /// <summary>
        /// 得到数据块
        ...
        public async Task<int> WriteDeviceBlock(string iDeviceName, params int[] iData)
        {
            return await WriteDeviceBlock(iDeviceName, iData.Length, iData);
        }
        /// <summary>
        /// 写入数据块
        /// </summary>
        ...
        private async Task<byte[]> TryExecution(byte[] iCommand, int minlength)
        {
            ...
            do
            {
                ...
                if (tCount < 0)
                {
                    //Console.WriteLine("I can't get the correct response.");
                    throw new Exception("I can't get the correct response.");
                }
